...
It can be combined with WatsonX.data Developer edition and use RustFS, MDS and Presto from there. Where commands differ between the local setup and WatsonX.data, both variants are shown.
...
Set environment variables and bring up the full stack. The platform includes Kafka (KRaft, 3 brokers), Schema Registry, ksqlDB, Kafka Connect, Spark, Hive Metastore, RustFS, PostgreSQL, and Presto.
...
show the tables of the application in postgresql
...
Merchant data is directly made available as a state topic in Kafa.
...
First upload `data/cities1000.txt` to `s3a://landing-bucket/geonames` in local RustFS:
...
Before we can use the Kafka Connect [Iceberg Connector](https://iceberg.apache.org/docs/nightly/kafka-connect/), we first have to create the `control-iceberg` control topic. 
